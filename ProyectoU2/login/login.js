...
const LS_LOGGED = "loggedUser";
const LS_PRODUCTS = "productos"; 

const loadArray = (key) => {
    try { return JSON.parse(localStorage.getItem(key) || "[]"); } catch { return []; }
};
const saveArray = (key, arr) => localStorage.setItem(key, JSON.stringify(arr));
const getLogged = () => JSON.parse(localStorage.getItem(LS_LOGGED) || "null");
const setLogged = (obj) => localStorage.setItem(LS_LOGGED, JSON.stringify(obj));
const clearLogged = () => localStorage.removeItem(LS_LOGGED);
...
// Estado en memoria
let users = loadArray(LS_USERS);
let products = loadArray(LS_PRODUCTS);
...
function renderProducts() {
    ...
    if (!container) return;
    container.innerHTML = '';
    products = loadArray(LS_PRODUCTS);
    if (!products.length) {
...
if (formulario_producto) {
    ...
    products = loadArray(LS_PRODUCTS);
        ...
        saveArray(LS_PRODUCTS, products);
